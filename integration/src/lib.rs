...
impl PolymeshTester {
  pub async fn new() -> Result<Self> {
    ...
    let ts = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH)
      .expect("now later then epoch").as_nanos();
    Ok(Self {
      api,
  ...
  pub async fn users(&mut self, names: &[&str]) -> Result<Vec<User>> {
    ...
    for (idx, user) in users.iter().enumerate() {
      ...
      calls.push(self.api
        .call()
        .identity()
        .cdd_register_did_with_cdd(user.account(), vec![], None)?.into());
    }
    ...
    for user in &users {
      ...
      calls.push(self.api
        .call()
        .balances()
        .transfer(user.account().into(), self.init_polyx)?.into());
    }
    // Execute batch.
    let _res = self.api.call().utility().batch(calls)?.execute(&mut self.cdd).await?;
    // TODO: process events
    self.load_dids(users.as_mut_slice()).await?;
    // Update cached users.
  ...
  pub async fn register_and_fund(&mut self, account: AccountId) -> Result<IdentityId> {
    let did = match self.get_did(account).await? {
      ...
      None => {
        ...
        let mut _res = self.api.call().utility().batch(vec![
          self.api
            .call()
            .identity()
            .cdd_register_did_with_cdd(account, vec![], None)?.into(),
          self.api
            .call()
            .balances()
            .transfer(account.into(), self.init_polyx)?.into()
          ])?
