...
use primitives::{Key, TransactionError, IdentityId, traits::{IdentityCurrency}};
	...
	where BalanceOf<T>: Send + Sync
{
	...
	fn validate(
		...
		let encoded_transactor = Key::try_from(who.encode()).map_err(|_| InvalidTransaction::BadProof)?;
		let imbalance;
