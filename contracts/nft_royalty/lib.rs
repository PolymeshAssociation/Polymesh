...
//! - `decoded_asset_metadata_value`: Returns the decoded metadata value ([`NFTArtistRules`]) for the given [`AssetId`].
...
use polymesh_ink::{
    ...
    AssetId, VenueDetails, VenueId, VenueType,
};
...
mod nft_royalty {
    ...
    pub enum Error {
        ...
        RoyaltyMetadataValueNotFound(AssetId),
        /// Royalty metadata key not found.
        RoyaltyMetadataKeyNotFound(AssetId),
        /// Trying to decode [`NFTArtistRules`] from [`AssetMetadataValue`] failed.
        FailedToDecodeMetadataValue(String),
        /// [`NFTOffer::purchase_asset_id`] can't be used as royalty payment.
        AssetIdNotAllowedForRoyalty(AssetId),
    ...
    pub struct NftRoyalty {
        ...
        /// The asset metadata key for each assetId.
        metadata_keys: Mapping<AssetId, u64>,
    }
    ...
    impl NftRoyalty {
        ...
        #[ink(message)]
        pub fn royalty_percentage(&mut self, asset_id: AssetId) -> Result<Perbill> {
            let nft_artist_rules = self.decoded_asset_metadata_value(asset_id)?;
            Ok(nft_artist_rules.royalty_percentage)
        }

        /// Returns the decoded metadata value ([`NFTArtistRules`]) for the given [`AssetId`].
        #[ink(message)]
        pub fn decoded_asset_metadata_value(&mut self, asset_id: AssetId) -> Result<NFTArtistRules> {
            let asset_metadata_value = self.asset_metadata_value(asset_id)?;
        ...
        pub fn create_transfer(
            ...
            let nft_artist_rules =
                self.decoded_asset_metadata_value(nft_transfer_details.collection_asset_id)?;
            ...
            api.add_and_affirm_instruction(Some(venue_id), legs.clone(), vec![royalty_portfolio].into_iter().collect::<BTreeSet<_>>())?;
            Ok(legs)
        }
        ...
        fn ensure_valid_transfer_values(
            ...
            if !nft_artist_rules.is_asset_id_allowed(&nft_offer.purchase_asset_id) {
                return Err(Error::AssetIdNotAllowedForRoyalty(nft_offer.purchase_asset_id));
            }
            Ok(())
        }
        ...
        fn setup_legs(
            ...
            let nfts = NFTs {
                asset_id: nft_transfer_details.collection_asset_id,
            ...
            let nft_payment_leg = Leg::Fungible {
                ...
                asset_id: nft_offer.purchase_asset_id,
        ...
        /// Returns the [`AssetMetadataKey`] for the given `asset_id`.
        fn asset_metadata_key(
            ...
            if let Some(key_id) = self.metadata_keys.get(asset_id) {
            ...
            let local_metadata_key = api
                ...
                .ok_or(Error::RoyaltyMetadataKeyNotFound(asset_id))?;

            // Add the key to the storage
            self.metadata_keys.insert(asset_id, &local_metadata_key.0);
            Ok(AssetMetadataKey::Local(local_metadata_key))
        }

        /// Returns the [`AssetMetadataValue`] for the given `asset_id`.
        fn asset_metadata_value(&mut self, asset_id: AssetId) -> Result<AssetMetadataValue> {
            let api = PolymeshInk::new()?;
            let metadata_key = self.asset_metadata_key(asset_id, &api)?;

            api.asset_metadata_value(asset_id, metadata_key)?
                .ok_or(Error::RoyaltyMetadataValueNotFound(asset_id))
        }
    ...
    pub mod types {
        ...
        impl NFTArtistRules {
            ...
            pub fn new(
                allowed_purchase_asset_ids: BTreeSet<AssetId>,
                royalty_percentage: Perbill,
                artist_identity: IdentityId,
            ) -> Self {
                Self {
                    allowed_purchase_asset_ids,
                    royalty_percentage,
                    artist_identity,
                }
            }

            /// Returns `true` if `asset_id` is in the [`NFTArtistRules::allowed_purchase_asset_ids`] set. Otherwise, returns `false`.
            pub fn is_asset_id_allowed(&self, asset_id: &AssetId) -> bool {
                self.allowed_purchase_asset_ids.contains(asset_id)
            }
        }
    }
}
