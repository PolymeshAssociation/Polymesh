...
function run_tests() {
    ...
    INTEGRATION_TEST=1 \
    cargo +nightly-2026-09-01 test --tests \
...
cargo +nightly-2026-09-01 profdata -- merge -sparse $(find . -name 'json5format-*.profraw') -o json5format.profdata

if [[ -v CIRCLECI ]]; then
    cargo +nightly-2026-09-01 cov -- export \
    $( get_tests_filenames ) \
    --format='lcov' \
    ...
    cargo +nightly-2026-09-01 cov -- report \
    $( get_tests_filenames ) \
    --instr-profile=json5format.profdata \
    --use-color \
    --ignore-filename-regex='/.cargo/registry/' \
