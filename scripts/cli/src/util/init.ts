...
export async function generateEntityFromUri(api: ApiPromise, uri: string) {
	...
	let accountRawNonce = (await api.query.system.account(entity.address))
		.nonce;
	let account_nonce = new BN(accountRawNonce.toString());
...
export async function generateRandomEntity(api: ApiPromise) {
	let entity = await generateEntityFromUri(
		api,
...
export async function blockTillPoolEmpty(api: ApiPromise) {
	...
	const unsub = await api.rpc.chain.subscribeNewHeads(async (header) => {
		...
		if (header.number.toNumber() > last_synced_block) {
			for (
				let i = last_synced_block + 1;
				i <= header.number.toNumber();
				i++
			) {
				...
				let new_block_ts = parseInt(
					JSON.stringify(
						timestamp_extrinsic["method"].args[0].toHuman()
					)
				);
...
export async function issueTokenPerDid(
	api: ApiPromise,
	accounts: KeyringPair[],
	ticker: Ticker,
	amount: Balance,
	...
	let tickerExist = ((await api.query.asset.tickers(
		ticker
			...
			(receipt) => {
				...
				if (receipt.isCompleted) {
					...
					if (receipt.isInBlock) {
						...
						const failed = receipt.findRecord(
							"system",
							"ExtrinsicFailed"
						);
						...
						if (failed) {
							...
							let message = "";
							const dispatchError = (failed.event
...
export async function generateOffchainKeys(api: ApiPromise, keyType: string) {
	...
	await api.rpc.author.insertKey(
		keyType,
		PHRASE,
		u8aToHex(newPair.publicKey)
	);
}

// Creates a Signatory Object
export async function signatory(
	api: ApiPromise,
	entity: KeyringPair,
	signer: KeyringPair
) {
...
export async function addComplianceRequirement(
	...
	let assetCompliance = ((await api.query.complianceManager.assetCompliances(
		ticker
...
export async function rejectInstruction(
	...
	const transaction = api.tx.settlement.rejectInstruction(
		instructionCounter,
		[getDefaultPortfolio(did)]
	);
...
export async function addInstruction(
	...
	if (ticker2 === null || ticker2 === undefined) {
		transaction = api.tx.settlement.addInstruction(
			venueCounter,
			0,
			null,
			null,
