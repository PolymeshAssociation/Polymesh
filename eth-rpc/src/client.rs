...
pub async fn connect(
    ...
    (
        ...
        RpcClient,
        LegacyRpcMethods<SrcChainConfig>,
    ),
    ClientError,
> {
...
impl Client {
    ...
    pub async fn subscribe_and_cache_new_blocks(
        ...
        self.subscribe_new_blocks(subscription_type, |block| async {
            ...
            self.block_provider
                .update_latest(Arc::new(block), subscription_type)
                .await;
            self.fee_history_provider
                .update_fee_history(&evm_block, &receipts)
                .await;
    ...
    pub async fn subscribe_and_cache_blocks(
        ...
        self.subscribe_past_blocks(range, |block| async move {
            ...
            self.receipt_provider
                .insert_block_receipts(&block, &ethereum_hash)
                .await?;
            Ok(())
        })
        .await?;
    ...
    pub async fn sync_state(
        ...
        let sync_state: sc_rpc::system::SyncState<SubstrateBlockNumber> = client
            .request("system_syncState", Default::default())
            .await?;
        Ok(sync_state)
    }
    ...
    pub async fn receipt_by_ethereum_hash_and_index(
        ...
        let substrate_hash = self
            .resolve_substrate_hash(ethereum_hash)
            .await
            .unwrap_or(*ethereum_hash);
        self.receipt_by_hash_and_index(&substrate_hash, transaction_index)
            .await
    }
    ...
    pub async fn resolve_substrate_hash(&self, ethereum_hash: &H256) -> Option<H256> {
        self.receipt_provider
            .get_substrate_hash(ethereum_hash)
            .await
    }
    ...
    pub async fn resolve_ethereum_hash(&self, substrate_hash: &H256) -> Option<H256> {
        self.receipt_provider
            .get_ethereum_hash(substrate_hash)
            .await
    }
    ...
    pub async fn trace_block_by_number(
        ...
        let traces = traces.into_iter().filter_map(|(index, trace)| {
            Some(TransactionTrace {
                tx_hash: hashes.remove(&(index as usize))?,
                trace,
            })
        });
