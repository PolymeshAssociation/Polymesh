...
impl FeeHistoryProvider {
    ...
    pub async fn update_fee_history(&self, block: &Block, receipts: &[ReceiptInfo]) {
        ...
        let block_number: SubstrateBlockNumber = block
            .number
            .try_into()
            .expect("Block number is always valid");
...
async fn test_update_fee_history() {
    ...
    let provider = FeeHistoryProvider {
        fee_history_cache: Arc::new(RwLock::new(BTreeMap::new())),
    };
    provider.update_fee_history(&block, &receipts).await;

    let fee_history_result = provider
        .fee_history(1, 200, Some(vec![0.0f64, 50.0, 100.0]))
        .await
        .unwrap();
