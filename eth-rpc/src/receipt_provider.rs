...
impl<B: BlockInfoProvider> ReceiptProvider<B> {
    ...
    pub fn is_before_earliest_block(&self, at: &BlockNumberOrTag) -> bool {
        match at {
            BlockNumberOrTag::U256(block_number) => self
                .receipt_extractor
                .is_before_earliest_block(block_number.as_u32()),
    ...
    pub async fn logs(&self, filter: Option<Filter>) -> anyhow::Result<Vec<Log>> {
        ...
        match (from_block, to_block, filter.block_hash) {
            ...
            (None, None, None) => {
                qb.push(" AND block_number = ")
                    .push_bind(latest_block.as_u64() as i64);
            }
        }
...
mod tests {
        ...
        let count: i64 = match block_hash {
            ...
            Some(hash) => {
                sqlx::query_scalar(&format!(
                    "SELECT COUNT(*) FROM {table} WHERE block_hash = ?"
                ))
                .bind(hash.as_ref())
                .fetch_one(pool)
                .await
    ...
    async fn test_insert_remove(pool: SqlitePool) -> anyhow::Result<()> {
        ...
        let row = provider
            .find_transaction(&receipts[0].1.transaction_hash)
            .await;
        assert_eq!(row, Some((block.hash, 0)));

        provider.remove(&[block_map]).await?;
        assert_eq!(
            count(&provider.pool, "transaction_hashes", Some(block.hash())).await,
            0
        );
    ...
    async fn test_prune(pool: SqlitePool) -> anyhow::Result<()> {
        ...
        assert_eq!(
            count(&provider.pool, "eth_to_substrate_blocks", None).await,
            n
        );
    ...
    async fn test_fork(pool: SqlitePool) -> anyhow::Result<()> {
        ...
        provider
            .insert(&fork_block, &receipts, &ethereum_hash_fork)
            .await?;
    ...
    async fn test_query_logs(pool: SqlitePool) -> anyhow::Result<()> {
        let provider = setup_sqlite_provider(pool).await;
        let block1 = MockBlockInfo {
            hash: H256::from([1u8; 32]),
            number: 1,
        };
        let block2 = MockBlockInfo {
        ...
        let logs = provider
            .logs(Some(Filter {
                from_block: Some(log2.block_number.into()),
                ..Default::default()
            }))
            .await?;
        assert_eq!(logs, vec![log2.clone()]);
        ...
        let logs = provider
            .logs(Some(Filter {
                from_block: Some(BlockTag::Latest.into()),
                ..Default::default()
            }))
            .await?;
        assert_eq!(logs, vec![log2.clone()]);
        ...
        let logs = provider
            .logs(Some(Filter {
                to_block: Some(log1.block_number.into()),
                ..Default::default()
            }))
            .await?;
        ...
        let logs = provider
            .logs(Some(Filter {
                block_hash: Some(log1.block_hash),
                ..Default::default()
            }))
            .await?;
        assert_eq!(logs, vec![log1.clone()]);
    ...
    async fn test_block_mapping_remove(pool: SqlitePool) -> anyhow::Result<()> {
        ...
        assert_eq!(
            provider.get_substrate_hash(&ethereum_hash2).await,
            Some(substrate_hash2)
        );
