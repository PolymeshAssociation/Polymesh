...
async fn prepare_evm_transactions<Client: EthRpcClient + Sync + Send>(
    ...
    let start_nonce = client
        .get_transaction_count(signer.address(), BlockTag::Latest.into())
        .await?;
...
async fn verify_transactions_in_single_block(
    ...
    if let Some(missing_hash) = expected_tx_hashes
        .iter()
        .find(|hash| !block_tx_hashes.contains(hash))
    {
        return Err(anyhow!(
            "Transaction {missing_hash:?} not found in block {block_number}"
        ));
    }
...
async fn test_deploy_and_call(client: Arc<WsClient>) -> anyhow::Result<()> {
    ...
    let nonce_after_deploy = client
        .get_transaction_count(account.address(), BlockTag::Latest.into())
        .await?;

    assert_eq!(
        nonce_after_deploy - nonce,
        U256::from(1),
        "Nonce should have increased by 1"
    );

    let initial_balance = client
        .get_balance(contract_address, BlockTag::Latest.into())
        .await?;
    assert_eq!(
...
async fn test_evm_blocks_should_match(client: Arc<WsClient>) -> anyhow::Result<()> {
    let (node_client, node_rpc_client, _) = client::connect(SharedResources::node_rpc_url())
        .await
        .unwrap();
    ...
    let evm_block_from_rpc_by_hash = client
    ...
    assert_eq!(
        evm_block_from_storage, evm_block_from_rpc_by_number,
        "EVM blocks should match"
    );
    assert_eq!(
        evm_block_from_storage, evm_block_from_rpc_by_hash,
        "EVM blocks should match"
    );
...
async fn test_block_hash_for_tag_with_proper_ethereum_block_hash_works(
    ...
    let account = Account::default();
    let balance = client
        .get_balance(account.address(), ethereum_block_hash.into())
        .await?;

    assert!(
        balance >= U256::zero(),
        "Balance should be retrievable with Ethereum hash"
    );
    assert_eq!(
        block_by_hash.hash, ethereum_block_hash,
...
async fn test_block_hash_for_tag_with_invalid_ethereum_block_hash_fails(
    ...
    let account = Account::default();
    let result = client
        .get_balance(account.address(), fake_eth_hash.into())
        .await;

    assert!(
        result.is_err(),
        "Should fail with non-existent Ethereum hash"
    );
...
async fn test_block_hash_for_tag_with_block_number_works(
    ...
    assert!(
        balance >= U256::zero(),
        "Balance should be retrievable with block number"
    );
    Ok(())
}
...
async fn test_block_hash_for_tag_with_block_tags_works(
    ...
    for tag in tags {
        let balance = client
            .get_balance(account.address(), tag.clone().into())
            .await?;

        assert!(
            balance >= U256::zero(),
            "Balance should be retrievable with tag {tag:?}"
        );
    }
...
async fn test_mixed_evm_substrate_transactions(client: Arc<WsClient>) -> anyhow::Result<()> {
    ...
    let (node_client, _, _) = client::connect(SharedResources::node_rpc_url())
        .await
        .unwrap();
...
async fn test_runtime_pallets_address_upload_code(client: Arc<WsClient>) -> anyhow::Result<()> {
    ...
    assert!(
        stored_code.is_some(),
        "Code with hash {code_hash:?} should exist in storage"
    );
    assert_eq!(
        stored_code.unwrap(),
        bytecode,
        "Stored code should match the uploaded bytecode"
    );
