...
benchmarks! {
    ...
    heartbeat {
        ...
        frame_support::debug::info!( "Heartbeat::heartbeat bench: keys={}, external_addresses={}", k, e);
    }: _(RawOrigin::None, input_heartbeat.clone(), signature)
    verify {
        let current_session = <pallet_session::Module<T>>::current_index();
        let raw_net_state = ImOnline::<T>::received_heartbeats(&current_session, &input_heartbeat.authority_index) ;
        let exp_raw_net_state = input_heartbeat.network_state.encode();
        assert_eq!( raw_net_state, Some(exp_raw_net_state));
    }
    ...
    validate_unsigned {
        ...
        frame_support::debug::info!( "Hearbeat::validate_bench: keys={}, external_addresses={}", k, e);
    }: {
    ...
    set_slashing_params {
        let params = OfflineSlashingParams{
            max_offline_percent: 20,
            constant: 6,
            max_slash_percent: 14
        };
    }: _(RawOrigin::Root, params.clone())
    verify {
        assert_eq!(SlashingParams::get(), params);
    }
}
