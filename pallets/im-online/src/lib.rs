...
use sp_runtime::{
	...
	RuntimeDebug,
	traits::{Convert, Member, Saturating, AtLeast32Bit, EnsureOrigin}, Perbill, PerThing,
...
use frame_support::{
	...
	traits::Get, ensure, weights::SimpleDispatchInfo
};
...
pub struct OfflineSlashingParams {
    /// Max percent of validators that can be offline without incurring slashing (default = 10)
    pub max_offline_percent: u32,
    /// Constant used in calculating slashing fraction (default = 3)
    pub constant: u32,
    /// Max percent slashed in critical case (default = 7)
...
decl_storage! {
    trait Store for Module<T: Trait> as ImOnline {
		...
		ReceivedHeartbeats get(fn received_heartbeats):
			double_map hasher(blake2_256) SessionIndex, hasher(blake2_256) AuthIndex => Option<Vec<u8>>;
        ...
        SlashingParams get(fn slashing_params) config(): OfflineSlashingParams;
    }
    add_extra_genesis {
...
impl<T: Trait> pallet_session::OneSessionHandler<T::AccountId> for Module<T> {
	...
	fn on_before_session_ending() {
		...
		if offenders.is_empty() {
			...
			let offence = UnresponsivenessOffence { session_index, validator_set_count, offenders, phantom: PhantomData {} };
...
pub struct UnresponsivenessOffence<T, Offender> {
