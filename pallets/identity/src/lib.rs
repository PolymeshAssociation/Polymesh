...
decl_module! {
    ...
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        ///
        /// # Weight
        /// `300_000 + 20_000 * signing_items.len()`
        ///
        /// # TODO
        ...
        #[weight = FunctionOf(
            |(_, _, items): (&T::AccountId, &Option<T::Moment>, &Vec<SigningItem>)| {
                30_000 + 2_000 * u32::try_from(items.len()).unwrap_or_default()
            },
        ...
        ///
        /// # Weight
        /// `30_000 + 2_000 * signers_to_remove.len()`
        #[weight = FunctionOf(
            |(items,): (&Vec<Signatory>,)| {
                30_000 + 2_000 * u32::try_from(items.len()).unwrap_or_default()
            },
        ...
        #[weight = SimpleDispatchInfo::FixedNormal(25_000)]
        ...
        ///
        /// # Weight
        /// `30_000 + 2_000 * claims.len()`
        #[weight = FunctionOf(
            |(claims,): (&Vec<BatchAddClaimItem<T::Moment>>,)| {
                30_000 + 2_000 * u32::try_from(claims.len()).unwrap_or_default()
            },
            DispatchClass::Normal,
            true
        )]
        ...
        #[weight = FunctionOf(
            |(_, call): (&IdentityId, &Box<T::Proposal>)| {
                20_000 + call.get_dispatch_info().weight
            },
            |(_, call): (&IdentityId, &Box<T::Proposal>)| {
        ...
        ///
        /// # Weight
        /// `20_000 + 1_000 * claims.len()`
        #[weight = FunctionOf(
            |(claims,): (&Vec<BatchRevokeClaimItem>,)| {
                20_000 + 1_000 * u32::try_from(claims.len()).unwrap_or_default()
            },
            DispatchClass::Normal,
            true
        )]
        ...
        ///
        /// # Weight
        /// `20_000 + 1_000 * permissions.len()`
        #[weight = FunctionOf(
            |(_, permissions): (&Signatory, &Vec<Permission>)| {
                20_000 + 1_000 * u32::try_from(permissions.len()).unwrap_or_default()
            },
            DispatchClass::Normal,
            true
        )]
        ...
        #[weight = SimpleDispatchInfo::FixedNormal(18_000)]
        ...
        ///
        /// # Weight
        /// `18_000 + 2_000 * auths.len()`
        #[weight = FunctionOf(
            |(auths,): (&Vec<(Signatory, AuthorizationData, Option<T::Moment>)>,)| {
                18_000 + 2_000 * u32::try_from(auths.len()).unwrap_or_default()
            },
            DispatchClass::Normal,
            true
        )]
        ...
        ///
        /// # Weight
        /// `10_000 + 15_000 * auth_ids.len()`
        #[weight = FunctionOf(
            |(auth_ids,): (&Vec<u64>,)| {
                10_000 + 15_000 * u32::try_from(auth_ids.len()).unwrap_or_default()
            },
            DispatchClass::Normal,
            true
        )]
        ...
        ///
        /// # Weight
        /// `20_000 + 2_000 * auths.len()`
        #[weight = FunctionOf(
            |(_, additional_keys): (&T::Moment, &Vec<SigningItemWithAuth>)| {
                20_000 + 2_000 * u32::try_from(additional_keys.len()).unwrap_or_default()
            },
            DispatchClass::Normal,
            true
        )]
