...
//! - `remove_signing_keys` - Removes specified signing keys of a DID if present.
...
//! - `batch_add_signing_key_with_authorization` - Adds signing keys to target identity `id`.
...
use polymesh_primitives::{
    ...
    Scope, Signatory, SigningKey, Ticker,
};
        ...
        build(|config: &GenesisConfig<T>| {
            ...
            for &(ref signer_id, did) in &config.signing_keys {
                ...
                <Module<T>>::deposit_event(RawEvent::SigningKeysAdded(
                    did,
                    [SigningKey::from_account_id(signer_id.clone())].to_vec(),
                ));
            }
        });
    }
...
decl_module! {
    ...
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        pub fn register_did(origin,
            uid: InvestorUid,
            signing_keys: Vec<SigningKey<T::AccountId>>) -> DispatchResult {
            let sender = ensure_signed(origin)?;
            Self::_register_did(sender.clone(), signing_keys, Some(ProtocolOp::IdentityRegisterDid))?;
        ...
        ///
        /// # Weight
        /// `30_000_000 + 60_000 * signing_keys.len()`
        #[weight = (
            (30_000_000 + 60_000 * u64::try_from(signing_keys.len()).unwrap_or_default()),
        ...
        pub fn cdd_register_did(
            ...
            signing_keys: Vec<SigningKey<T::AccountId>>
        ) -> DispatchResult {
        ...
        ///
        /// # Weight
        /// `25_000_000 + 60_000 * signers_to_remove.len()`
        #[weight = (
            25_000_000 + 60_000 * u64::try_from(signers_to_remove.len()).unwrap_or_default(),
            DispatchClass::Normal,
            Pays::Yes
        )]
        pub fn remove_signing_keys(origin, signers_to_remove: Vec<Signatory<T::AccountId>>) -> DispatchResult {
            let sender = ensure_signed(origin)?;
            ...
            <DidRecords<T>>::mutate(did, |record| {
                (*record).remove_signing_keys(&signers_to_remove);
            });

            Self::deposit_event(RawEvent::SigningKeysRemoved(did, signers_to_remove));
            Ok(())
        }
        ...
        ///
        /// # Weight
        /// `15_000_000 + 1_000_000 * claims.len()`
        #[weight =(
            15_000_000 + 1_000_000 * u64::try_from(claims.len()).unwrap_or_default(),
            DispatchClass::Normal,
            Pays::Yes
        )]
        ...
        #[weight =(
            5_000_000 + proposal.get_dispatch_info().weight,
        ...
        ///
        /// # Weight
        /// `2_000_000 + 150_000 * claims.len()`
        #[weight = (
            2_000_000 + 150_000 * u64::try_from(claims.len()).unwrap_or_default(),
            DispatchClass::Normal,
            Pays::Yes
        )]
        ...
        ///
        /// # Weight
        /// `4_000_000 + 300_000 * permissions.len()`
        #[weight =(
            4_000_000 + 300_000 * u64::try_from(permissions.len()).unwrap_or_default(),
            DispatchClass::Normal,
            Pays::Yes
        )]
        pub fn set_permission_to_signer(
            ...
            match signer {
                ...
                _ if record.signing_keys.iter().any(|si| si.signer == signer) => Self::update_signing_key_permissions(did, &signer, permissions),
        ...
        ///
        /// # Weight
        /// `10_000_000 + 50_000 * auths.len()`
        #[weight =(
            10_000_000 + 500_000 * u64::try_from(auths.len()).unwrap_or_default(),
            DispatchClass::Normal,
            Pays::Yes
        )]
        ...
        ///
        /// # Weight
        /// `25_000_000 + 1_000_000 * auths.len()`
        #[weight =(
            25_000_000 + 1_000_000 * u64::try_from(auth_identifiers.len()).unwrap_or_default(),
            DispatchClass::Normal,
            Pays::Yes
        )]
        ...
        pub fn accept_authorization(
            ...
            let auth_signer = if <Authorizations<T>>::contains_key(&signer_did, auth_id) {
                let auth = <Authorizations<T>>::get(&signer_did, auth_id);
                Some((auth, signer_did))
            } else if <Authorizations<T>>::contains_key(&signer_key, auth_id) {
                let auth = <Authorizations<T>>::get(&signer_key, auth_id);
                Some((auth, signer_key))
            } else {
                None
            };

            let (auth, signer) = auth_signer
        ...
        ///
        /// # Weight
        /// `60_000_000 + 5_000_000 * auth_ids.len()`
        #[weight =(
            60_000_000 + 5_000_000 * u64::try_from(auth_ids.len()).unwrap_or_default(),
            DispatchClass::Normal,
            Pays::Yes
        )]
        ...
        ///
        /// # Weight
        /// `25_000_000 + 2_000_000 * auths.len()`
        #[weight = (
            25_000_000 + 2_000_000 * u64::try_from(additional_keys.len()).unwrap_or_default(),
            DispatchClass::Normal,
            Pays::Yes
        )]
        pub fn batch_add_signing_key_with_authorization(
            origin,
            ...
            for si_with_auth in additional_keys.iter() {
                let si = &si_with_auth.signing_key;
            ...
            <DidRecords<T>>::mutate(id, |record| {
                (*record).add_signing_keys(&additional_keys_si[..]);
            });
            <OffChainAuthorizationNonce>::mutate(id, |offchain_nonce| {
                *offchain_nonce = authorization.nonce + 1;
            });

            Self::deposit_event(RawEvent::SigningKeysAdded(id, additional_keys_si));
...
impl<T: Trait> Module<T> {
    ...
    fn update_signing_key_permissions(
        ...
        <DidRecords<T>>::mutate(target_did, |record| {
            if let Some(mut signing_key) = (*record)
                .signing_keys
                .iter()
                .find(|si| si.signer == *signer)
                .cloned()
            {
                swap(&mut signing_key.permissions, &mut permissions);
                (*record).signing_keys.retain(|si| si.signer != *signer);
                (*record).signing_keys.push(signing_key.clone());
                new_s_item = Some(signing_key);
            }
        });
        ...
        match signer {
            ...
            _ => {
                ...
                !Self::is_did_frozen(did)
                    && record.signing_keys.iter().any(|si| si.signer == *signer)
            }
        }
    }
    ...
    pub fn _register_did(
        ...
        Self::deposit_event(RawEvent::DidCreated(did, sender, signing_keys));
        Ok(did)
    }
