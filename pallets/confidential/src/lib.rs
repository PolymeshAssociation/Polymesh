...
decl_storage! {
    trait Store for Module<T: Trait> as Confidential {
        ...
        pub RangeProofs get(fn range_proof): double_map hasher(twox_64_concat) IdentityId, hasher(twox_64_concat) Ticker => Option<TickerRangeProof>;
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        pub fn add_range_proof(origin, ticker: Ticker, secret_value: u64, max_two_exp: u8 ) -> DispatchResult {
            let prover_key = AccountKey::try_from(ensure_signed(origin)?.encode())?;
            let prover_id = Context::current_identity_or::<Identity<T>>(&prover_key)?;
            ...
            <RangeProofs>::insert(&prover_id, &ticker, ticker_range_proof);
            Ok(())
        }

        #[weight = SimpleDispatchInfo::FixedNormal(400_000)]
        pub fn verify_range_proof(origin, id: IdentityId, ticker: Ticker) -> DispatchResult {
            let verifier_key = AccountKey::try_from(ensure_signed(origin)?.encode())?;
            let verifier_id = Context::current_identity_or::<Identity<T>>(&verifier_key)?;

            let trp = Self::range_proof(&id, &ticker)
            ...
            <RangeProofVerifications>::insert((id,ticker), verifier_id, true);
            Ok(())
        }
    }
}
