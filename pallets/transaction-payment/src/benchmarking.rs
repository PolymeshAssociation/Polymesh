...
fn assert_last_event<T: Config>(
    generic_event: <T as pallet_transaction_payment::Config>::RuntimeEvent,
) {
...
mod benchmarks {
    ...
    fn charge_transaction_payment() {
        ...
        let amount_to_endow = 1_000_000_000u32.into();
        let tip = 0u32.into();
        ...
        post_info
            .actual_weight
            .as_mut()
            .map(|w| w.saturating_accrue(extension_weight));
        let actual_fee = TransactionPallet::<T>::compute_actual_fee(10, &info, &post_info, tip);
        assert_last_event::<T>(
