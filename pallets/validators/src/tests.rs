...
use pallet_staking::{asset, ledger::StakingLedgerInspect};
...
fn bond_with_duplicate_vote_should_be_ignored_by_election_provider() {
	ExtBuilder::default()
      ...
      // POLYMESH NOTE: Changed the order.
...
fn test_multi_page_payout_stakers_by_page() {
    ...
    // POLYMESH NOTE: Changed order.
		assert!(matches!(
			staking_events_since_last_call().as_slice(),
			&[
				Event::PayoutStarted { era_index: 1, validator_stash: 11, page: 0, next: Some(1) },
				..,
				Event::Rewarded { stash: 1082, dest: RewardDestination::Stash, amount: 113 },
				Event::Rewarded { stash: 1053, dest: RewardDestination::Stash, amount: 110 },
			]
		));
    ...
    // POLYMESH NOTE: Changed order.
		assert!(matches!(
			...
			&[
				...
				Event::Rewarded { stash: 1059, dest: RewardDestination::Stash, amount: 111 },
				Event::Rewarded { stash: 1087, dest: RewardDestination::Stash, amount: 114 },
				..
			]
		));
...
// POLYMESH NOTE: This is not panicing for us.
#[ignore]
#[test]
#[should_panic]
...
// POLYMESH NOTE: This is not panicing for us, since our voter list is not sorted.
#[ignore]
#[test]
#[should_panic]
#[cfg(debug_assertions)]
...
mod sorted_list_provider {
	...
	fn re_validate_does_not_change_counters_or_list() {
      ...
      // POLYMESH NOTE: Changed the order.
			assert_eq!(<Test as Config>::VoterList::iter().collect::<Vec<_>>(), vec![31, 21, 11]);
...
mod getters {
	...
	use pallet_staking::{
		...
		Forcing, Nominations, Nominators, SlashRewardFraction, SlashingSpans,
