...
fn force_unstake_works() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(Staking::force_unstake(Origin::signed(account_from(11)), account_from(11)), BadOrigin);
		// We now force them to unstake
		assert_ok!(Staking::force_unstake(Origin::ROOT, account_from(11)));
		// No longer bonded.
		assert_eq!(Staking::bonded(&account_from(11)), None);
		// Transfer works.
		assert_ok!(Balances::transfer(Origin::signed(account_from(11)), account_from(1), 10));
	});
}
...
fn basic_setup_works() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			...
			Some(StakingLedger { stash: account_from(11), total: 1000, active: 1000, unlocking: vec![], last_reward: None })
		);
		// Account 20 controls the stash from account 21, which is 200 * balance_factor units
		assert_eq!(
			Staking::ledger(&account_from(20)),
			Some(StakingLedger { stash: account_from(21), total: 1000, active: 1000, unlocking: vec![], last_reward: None })
		);
		...
		assert_eq!(
			...
			Some(StakingLedger { stash: account_from(101), total: 500, active: 500, unlocking: vec![], last_reward: None })
		);
		assert_eq!(Staking::nominators(account_from(101)).unwrap().targets, vec![account_from(11), account_from(21)]);
		...
		assert_eq!(
			...
			Exposure {
				total: 1125,
				own: 1000,
				others: vec![ IndividualExposure { who: account_from(101), value: 125 }]
			},
		);
		assert_eq!(
			...
			Exposure {
				total: 1375,
				own: 1000,
				others: vec![ IndividualExposure { who: account_from(101), value: 375 }]
			},
		);
		// initial slot_stake
...
fn change_controller_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Staking::set_controller(Origin::signed(account_from(11)), account_from(5)));
		...
		assert_ok!(Staking::validate(Origin::signed(account_from(5)), ValidatorPrefs::default()));
	})
}
...
fn rewards_should_work() {
	...
	ExtBuilder::default().nominate(true).build().execute_with(|| {
		...
		assert_eq!(Balances::total_balance(&account_from(100)), init_balance_100);
		assert_eq!(Balances::total_balance(&account_from(101)), init_balance_101);
		assert_eq_uvec!(Session::validators(), vec![account_from(11), account_from(21)]);
		assert_eq!(Staking::eras_reward_points(Staking::active_era().unwrap().index), EraRewardPoints {
			total: 50*3,
			individual: vec![(account_from(11), 100), (account_from(21), 50)].into_iter().collect(),
		});
		...
		assert_eq_error_rate!(Balances::total_balance(&account_from(10)), init_balance_10 + part_for_10 * total_payout_0*2/3, 2);
		assert_eq_error_rate!(Balances::total_balance(&account_from(11)), init_balance_11, 2);
		assert_eq_error_rate!(Balances::total_balance(&account_from(20)), init_balance_20 + part_for_20 * total_payout_0*1/3, 2);
		assert_eq_error_rate!(Balances::total_balance(&account_from(21)), init_balance_21, 2);
		assert_eq_error_rate!(
		...
		assert_eq_error_rate!(Balances::total_balance(&account_from(101)), init_balance_101, 2);
		...
		assert_eq_error_rate!(Balances::total_balance(&account_from(10)), init_balance_10 + part_for_10 * (total_payout_0 * 2/3 + total_payout_1), 2);
		assert_eq_error_rate!(Balances::total_balance(&account_from(11)), init_balance_11, 2);
		assert_eq_error_rate!(Balances::total_balance(&account_from(20)), init_balance_20 + part_for_20 * total_payout_0 * 1/3, 2);
		assert_eq_error_rate!(Balances::total_balance(&account_from(21)), init_balance_21, 2);
		assert_eq_error_rate!(
...
fn staking_should_work() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq_uvec!(validator_controllers(), vec![account_from(20), account_from(10)]);

			// put some money in account that we'll use.
			for i in 1..5 { let _ = Balances::make_free_balance_be(&account_from(i), 2000); }

			// --- Block 2:
			start_session(2);
			// add a new candidate for being a validator. account 3 controlled by 4.
			assert_ok!(Staking::bond(Origin::signed(account_from(3)), account_from(4), 1500, RewardDestination::Controller));
			let current_era_at_bond = Staking::current_era();
            // Add validator in the potential validator list 
            assert_ok!(Staking::add_potential_validator(Origin::system(frame_system::RawOrigin::Root), account_from(3)));
            // Validate 
			assert_ok!(Staking::validate(Origin::signed(account_from(4)), ValidatorPrefs::default()));
			...
			assert_eq_uvec!(validator_controllers(), vec![account_from(20), account_from(4)]);
...
fn less_than_needed_candidates_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq_uvec!(validator_controllers(), vec![account_from(30), account_from(20), account_from(10)]);
...
fn no_candidate_emergency_condition() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			// initial validators
			assert_eq_uvec!(validator_controllers(), vec![account_from(10), account_from(20), account_from(30), account_from(40)]);
...
fn nominating_and_rewards_should_work() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq_uvec!(validator_controllers(), vec![account_from(40), account_from(30)]);

			// Set payee to controller
			assert_ok!(Staking::set_payee(Origin::signed(account_from(10)), RewardDestination::Controller));
			assert_ok!(Staking::set_payee(Origin::signed(account_from(20)), RewardDestination::Controller));
			assert_ok!(Staking::set_payee(Origin::signed(account_from(30)), RewardDestination::Controller));
			assert_ok!(Staking::set_payee(Origin::signed(account_from(40)), RewardDestination::Controller));
			...
			assert_ok!(Staking::bond(Origin::signed(account_from(1)), account_from(2), 1000, RewardDestination::Controller));
            // Add identity to the stash 1 
            create_did_and_add_claim(account_from(1));
            // nominate after did has the valid claim
			assert_ok!(Staking::nominate(Origin::signed(account_from(2)), vec![account_from(11), account_from(21), account_from(31)]));
			// 4 will nominate for 10, 20, 40
			assert_ok!(Staking::bond(Origin::signed(account_from(3)), account_from(4), 1000, RewardDestination::Controller));
            // Add identity to the stash 3 
            create_did_and_add_claim(account_from(3));
            // nominate after did has the valid claim
			assert_ok!(Staking::nominate(Origin::signed(account_from(4)), vec![account_from(11), account_from(21), account_from(41)]));
			...
			mock::make_all_reward_payment(0);
			assert_eq!(Balances::total_balance(&account_from(40)), 1 + total_payout_0 / 2);
			assert_eq!(Balances::total_balance(&account_from(30)), 1 + total_payout_0 / 2);
			...
			assert_eq!(
				...
				Exposure {
					...
					own: 1000,
					others: vec![
						IndividualExposure { who: account_from(3), value: 400 },
						IndividualExposure { who: account_from(1), value: 400 },
					]
				},
			);
			assert_eq!(
				...
				Exposure {
					...
					own: 1000,
					others: vec![
						IndividualExposure { who: account_from(3), value: 600 },
						IndividualExposure { who: account_from(1), value: 600 },
					]
				},
			);
...
fn nominators_also_get_slashed() {
	...
	ExtBuilder::default().nominate(false).build().execute_with(|| {
		...
		assert_ok!(Staking::bond(Origin::signed(account_from(1)), account_from(2), nominator_stake, RewardDestination::default()));
		// Add identity to the stash 1 
        create_did_and_add_claim(account_from(1));
        // nominate after did has the valid claim
        assert_ok!(Staking::nominate(Origin::signed(account_from(2)), vec![account_from(20), account_from(10)]));
		...
		let expo = Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(11));
		let slash_value = 50;
		...
		assert_eq!(Balances::total_balance(&account_from(11)), initial_balance - validator_slash);
		assert_eq!(Balances::total_balance(&account_from(2)), initial_balance - nominator_slash);
		check_exposure_all(Staking::active_era().unwrap().index);
		check_nominator_all(Staking::active_era().unwrap().index);
...
fn double_staking_should_fail() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(
			Staking::bond(Origin::signed(account_from(1)), account_from(2), arbitrary_value,
			RewardDestination::default())
		);
		// 4 = not used so far, 1 stashed => not allowed.
		assert_noop!(
			Staking::bond(Origin::signed(account_from(1)), account_from(4), arbitrary_value,
			RewardDestination::default()), Error::<Test>::AlreadyBonded,
		);
		// 1 = stashed => attempting to nominate should fail.
		assert_noop!(Staking::nominate(Origin::signed(account_from(1)), vec![account_from(1)]), Error::<Test>::NotController);
		// 2 = controller  => nominating should work.
		assert_ok!(Staking::nominate(Origin::signed(account_from(2)), vec![account_from(1)]));
	});
}
...
fn double_controlling_should_fail() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			Staking::bond(Origin::signed(account_from(3)), account_from(2), arbitrary_value, RewardDestination::default()),
...
fn cannot_transfer_staked_balance() {
	...
	ExtBuilder::default().nominate(false).build().execute_with(|| {
		...
		assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(11)).total, 1000);
		...
		assert_ok!(Balances::transfer(Origin::signed(account_from(11)), account_from(20), 1));
	});
}
...
fn cannot_transfer_staked_balance_2() {
	...
	ExtBuilder::default().nominate(false).fair(true).build().execute_with(|| {
		...
		assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(21)).total, 1000);
		// Confirm account 21 can transfer at most 1000
		assert_noop!(
			Balances::transfer(Origin::signed(account_from(21)), account_from(20), 1001),
			BalancesError::<Test>::LiquidityRestrictions
		);
		assert_ok!(Balances::transfer(Origin::signed(account_from(21)), account_from(20), 1000));
	});
}
...
fn cannot_reserve_staked_balance() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Balances::free_balance(account_from(11)), 1000);
		// Confirm account 11 (via controller 10) is totally staked
		assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(11)).own, 1000);
		// Confirm account 11 cannot transfer as a result
		assert_noop!(
...
fn reward_destination_works() {
	...
	ExtBuilder::default().nominate(false).build().execute_with(|| {
		...
		assert_eq!(Staking::ledger(&account_from(10)), Some(StakingLedger {
		...
		assert_eq!(Balances::free_balance(account_from(11)), 1000 + total_payout_0);
		...
		assert_eq!(Balances::free_balance(account_from(11)), 1000 + total_payout_0 + total_payout_1);
		// Record this value
		...
		assert_eq!(Staking::payee(&account_from(11)), RewardDestination::Controller);
		...
		assert_eq!(Balances::free_balance(account_from(11)), recorded_stash_balance);
	});
}
...
fn validator_payment_prefs_work() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		<Validators<Test>>::insert(&account_from(11), ValidatorPrefs {
		...
		let total_payout_1 = current_total_payout_for_duration(3000);
		assert!(total_payout_1 > 100); // Test is meaningful if reward something
		let exposure_1 = Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(11));
		<Module<Test>>::reward_by_ids(vec![(account_from(11), 1)]);
		...
		assert_eq_error_rate!(Balances::total_balance(&account_from(10)), balance_era_1_10 + reward_of_10, 2);
		assert_eq_error_rate!(Balances::total_balance(&account_from(100)), balance_era_1_100 + reward_of_100, 2);
...
fn bond_extra_works() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Staking::bond_extra(Origin::signed(account_from(11)), u128::max_value()));
...
fn bond_extra_and_withdraw_unbonded_works() {
	...
	ExtBuilder::default().nominate(false).build().execute_with(|| {
		...
		assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(11)), Exposure { total: 1000, own: 1000, others: vec![] });
		...
		assert_ne!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(11)), Exposure { total: 1000 + 100, own: 1000 + 100, others: vec![] });
		...
		assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(11)), Exposure { total: 1000 + 100, own: 1000 + 100, others: vec![] });

		// Unbond almost all of the funds in stash.
		Staking::unbond(Origin::signed(account_from(10)), 1000).unwrap();
		assert_eq!(Staking::ledger(&account_from(10)), Some(StakingLedger {
			stash: account_from(11), total: 1000 + 100, active: 100, unlocking: vec![UnlockChunk{ value: 1000, era: 2 + 3}], last_reward: None })
		);
		...
		assert_eq!(Staking::ledger(&account_from(10)), Some(StakingLedger {
			stash: account_from(11), total: 100, active: 100, unlocking: vec![], last_reward: None }));
	})
}
...
fn too_many_unbond_calls_should_not_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Staking::unbond(Origin::signed(account_from(10)), 1));
		// can't do more.
		assert_noop!(Staking::unbond(Origin::signed(account_from(10)), 1), Error::<Test>::NoMoreChunks);
		...
		// Can add again.
		assert_ok!(Staking::unbond(Origin::signed(account_from(10)), 1));
		assert_eq!(Staking::ledger(&account_from(10)).unwrap().unlocking.len(), 2);
	})
}
...
fn reward_to_stake_works() {
	ExtBuilder::default().nominate(false).fair(false).build().execute_with(|| {
		...
		assert!(<Validators<Test>>::contains_key(&account_from(11)) && <Validators<Test>>::contains_key(&account_from(21)));

		assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(11)).total, 1000);
		assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(21)).total, 2000);
		...
		ErasStakers::<Test>::insert(0, account_from(21), Exposure { total: 69, own: 69, others: vec![] });

		// Now lets lower account 20 stake
		assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(21)).total, 69);
		<Ledger<Test>>::insert(&account_from(20), StakingLedger { stash: account_from(21), total: 69, active: 69, unlocking: vec![], last_reward: None });
		...
		// -- new infos
		assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(11)).total, 1000 + total_payout_0 / 2);
		assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(21)).total, 69 + total_payout_0 / 2);
...
fn on_free_balance_zero_stash_removes_validator() {
	...
	ExtBuilder::default().existential_deposit(10).build().execute_with(|| {
		...
		assert_ok!(Staking::set_payee(Origin::signed(account_from(10)), RewardDestination::Stash));
...
fn on_free_balance_zero_stash_removes_nominator() {
	...
	ExtBuilder::default().existential_deposit(10).build().execute_with(|| {
		// Make 10 a nominator
		assert_ok!(Staking::nominate(Origin::signed(account_from(10)), vec![account_from(20)]));
...
fn switching_roles() {
	...
	ExtBuilder::default().nominate(false).build().execute_with(|| {
		...
		for i in &[10, 20] { assert_ok!(Staking::set_payee(Origin::signed(account_from(*i)), RewardDestination::Controller)); }

		assert_eq_uvec!(validator_controllers(), vec![account_from(20), account_from(10)]);

		// put some money in account that we'll use.
		for i in 1..7 { let _ = Balances::deposit_creating(&account_from(i), 5000); }

		// add 2 nominators
		assert_ok!(Staking::bond(Origin::signed(account_from(1)), account_from(2), 2000, RewardDestination::Controller));
        // create identity and add cdd claim
        create_did_and_add_claim(account_from(1));
        // nominate after having the right cdd claim
		assert_ok!(Staking::nominate(Origin::signed(account_from(2)), vec![account_from(11), account_from(5)]));

		assert_ok!(Staking::bond(Origin::signed(account_from(3)), account_from(4), 500, RewardDestination::Controller));
        // create identity and add cdd claim
        create_did_and_add_claim(account_from(3));
        // nominate after having the right cdd claim
		assert_ok!(Staking::nominate(Origin::signed(account_from(4)), vec![account_from(21), account_from(1)]));

		// add a new validator candidate
		assert_ok!(Staking::bond(Origin::signed(account_from(5)), account_from(6), 1000, RewardDestination::Controller));
        // create identity and add cdd claim
        create_did_and_add_claim(account_from(5));
        // add in to potential validator list
        assert_ok!(Staking::add_potential_validator(Origin::system(frame_system::RawOrigin::Root), account_from(5)));
        // add validator to validate
		assert_ok!(Staking::validate(Origin::signed(account_from(6)), ValidatorPrefs::default()));

		start_era(1);

		// with current nominators 10 and 5 have the most stake
		assert_eq_uvec!(validator_controllers(), vec![account_from(6), account_from(10)]);

		// 2 decides to be a validator. Consequences:
        // add in to potential validator list
        assert_ok!(Staking::add_potential_validator(Origin::system(frame_system::RawOrigin::Root), account_from(1)));
        // add validator to validate
		assert_ok!(Staking::validate(Origin::signed(account_from(2)), ValidatorPrefs::default()));
		// new stakes:
		// 10: 1000 self vote
		...
		assert_eq_uvec!(validator_controllers(), vec![account_from(2), account_from(20)]);
...
fn wrong_vote_is_null() {
	ExtBuilder::default().nominate(false).validator_pool(true).build().execute_with(|| {
		...
		// put some money in account that we'll use.
		for i in 1..3 { let _ = Balances::deposit_creating(&account_from(i), 5000); }

		// add 1 nominators
		assert_ok!(Staking::bond(Origin::signed(account_from(1)), account_from(2), 2000, RewardDestination::default()));
        create_did_and_add_claim(account_from(1));
		assert_ok!(Staking::nominate(Origin::signed(account_from(2)), vec![
			account_from(11), account_from(21), 			// good votes
			account_from(1), account_from(2), account_from(15), account_from(1000), account_from(25)  // crap votes. No effect.
		]));
...
fn bond_with_little_staked_value_bounded() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// Stingy validator.
			assert_ok!(Staking::bond(Origin::signed(account_from(1)), account_from(2), 1, RewardDestination::Controller));
			...
			// 2 is elected.
			assert_eq_uvec!(validator_controllers(), vec![account_from(20), account_from(10), account_from(2)]);
			// And has minimal stake
			assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(2)).total, 0);

			// Old ones are rewarded.
			assert_eq!(Balances::free_balance(account_from(10)), init_balance_10 + total_payout_0 / 3);
...
fn phragmen_should_not_overflow_validators() {
	ExtBuilder::default().nominate(false).build().execute_with(|| {
		...
		assert_eq_uvec!(validator_controllers(), vec![account_from(4), account_from(2)]);

		// This test will fail this. Will saturate.
		// check_exposure_all();
		assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(3)).total, u64::max_value().into());
		assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(5)).total, u64::max_value().into());
	})
}
...
fn reward_validator_slashing_validator_doesnt_overflow() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Staking::payout_validator(Origin::signed(account_from(10)), 0));
		...
		// it is 0.
		Staking::bond(Origin::signed(account_from(2)), account_from(20000), stake - 1, RewardDestination::default()).unwrap();
		// Override exposure of 11
		ErasStakers::<Test>::insert(0, account_from(11), Exposure {
			total: stake,
			own: 1,
			others: vec![ IndividualExposure { who: account_from(2), value: stake - 1 }]
		});
		...
		on_offence_now(
			&[
				OffenceDetails {
					offender: (account_from(11), Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(11))),
					reporters: vec![],
				},
			],
...
fn reward_from_authorship_event_handler_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(<pallet_authorship::Module<Test>>::author(), account_from(11));
		...
		assert_eq!(
			...
			EraRewardPoints {
				individual: vec![(account_from(11), 20 + 2 * 2 + 1), (account_from(21), 1)].into_iter().collect(),
				total: 26,
			},
		);
	})
...
fn add_reward_points_fns_works() {
	ExtBuilder::default().build().execute_with(|| {
		// Not mandatory but must be coherent with rewards
		assert_eq!(Session::validators(), vec![account_from(21), account_from(11)]);
		...
		assert_eq!(
			ErasRewardPoints::<Test>::get(Staking::active_era().unwrap().index),
			EraRewardPoints {
				individual: vec![(account_from(11), 4), (account_from(21), 2)].into_iter().collect(),
				total: 6,
			},
		);
	})
...
fn reporters_receive_their_slice() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(11)).total, initial_balance);
...
fn subsequent_reports_in_same_span_pay_out_less() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Balances::free_balance(account_from(1)), 10 + prior_payout + reward);
		assert_ledger_consistent(11);
	});
}
...
fn invulnerables_are_not_slashed() {
	...
	ExtBuilder::default().invulnerables(vec![account_from(11)]).build().execute_with(|| {
		assert_eq!(Balances::free_balance(account_from(11)), 1000);
		assert_eq!(Balances::free_balance(account_from(21)), 2000);

		let exposure = Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(21));
		...
		on_offence_now(
			&[
				...
				OffenceDetails {
					offender: (account_from(21), Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(21))),
					reporters: vec![],
				},
			],
		...
		assert_eq!(Balances::free_balance(account_from(21)), 2000 - (2 * initial_balance / 10));
...
fn garbage_collection_after_slashing() {
	ExtBuilder::default().existential_deposit(2).build().execute_with(|| {
		...
		assert_eq!(<Staking as crate::Store>::SpanSlash::get(&(account_from(11), 0)).amount_slashed(), &25_600);
		...
		assert_eq!(<Staking as crate::Store>::SpanSlash::get(&(account_from(11), 0)).amount_slashed(), &0);
	})
}
...
fn garbage_collection_on_window_pruning() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let exposure = Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(11));
		assert_eq!(Balances::free_balance(account_from(101)), 2000);
		let nominated_value = exposure.others.iter().find(|o| o.who == account_from(101)).unwrap().value;
		...
		assert_eq!(Balances::free_balance(account_from(101)), 2000 - (nominated_value / 10));

		assert!(<Staking as crate::Store>::ValidatorSlashInEra::get(&now, &account_from(11)).is_some());
		assert!(<Staking as crate::Store>::NominatorSlashInEra::get(&now, &account_from(101)).is_some());
		...
		assert!(<Staking as crate::Store>::ValidatorSlashInEra::get(&now, &account_from(11)).is_none());
		assert!(<Staking as crate::Store>::NominatorSlashInEra::get(&now, &account_from(101)).is_none());
	})
}
...
fn slashing_nominators_by_span_max() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let exposure_11 = Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(11));
		let exposure_21 = Staking::eras_stakers(Staking::active_era().unwrap().index, account_from(21));
		assert_eq!(Balances::free_balance(account_from(101)), 2000);
		let nominated_value_11 = exposure_11.others.iter().find(|o| o.who == account_from(101)).unwrap().value;
		let nominated_value_21 = exposure_21.others.iter().find(|o| o.who == account_from(101)).unwrap().value;
		...
		assert_eq!(Balances::free_balance(account_from(101)), 2000 - slash_1_amount);
		...
		assert_eq!(Balances::free_balance(account_from(101)), 2000 - slash_2_amount);
...
fn slashes_are_summed_across_spans() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(get_span(account_from(21)).iter().collect::<Vec<_>>(), expected_spans);
...
fn remove_deferred() {
	ExtBuilder::default().slash_defer_duration(2).build().execute_with(|| {
		...
		assert_eq!(Balances::free_balance(account_from(101)), 2000 - actual_slash);
	})
}
...
fn slash_kicks_validators_not_nominators() {
	ExtBuilder::default().build().execute_with(|| {
		...
		// re-registers.
		let last_slash = <Staking as Store>::SlashingSpans::get(&account_from(11)).unwrap().last_nonzero_slash();
...
fn claim_reward_at_the_last_era_and_no_double_claim_and_invalid_claim() {
	...
	ExtBuilder::default().nominate(true).build().execute_with(|| {
		...
		assert_ok!(Staking::payout_validator(Origin::signed(account_from(10)), 1));
		assert_ok!(Staking::payout_validator(Origin::signed(account_from(10)), 2));
		assert_noop!(
		...
		assert_noop!(
			Staking::payout_nominator(Origin::signed(account_from(100)), 0, vec![(account_from(11), 0)]),
			// Fail: Era out of history
			Error::<Test>::InvalidEraToReward
		);
		assert_ok!(Staking::payout_nominator(Origin::signed(account_from(100)), 1, vec![(account_from(11), 0)]));
		assert_ok!(Staking::payout_nominator(Origin::signed(account_from(100)), 2, vec![(account_from(11), 0)]));
		assert_noop!(
			Staking::payout_nominator(Origin::signed(account_from(100)), 2, vec![(account_from(11), 0)]),
			// Fail: Double claim
			Error::<Test>::InvalidEraToReward
		);
		assert_noop!(
			Staking::payout_nominator(Origin::signed(account_from(100)), active_era, vec![(account_from(11), 0)]),
			// Fail: Era not finished yet
			Error::<Test>::InvalidEraToReward
		);
...
fn test_max_nominator_rewarded_per_validator_and_cant_steal_someone_else_reward() {
	...
	ExtBuilder::default().build().execute_with(|| {
		for i in 0..=<Test as Trait>::MaxNominatorRewardedPerValidator::get() {
			...
			assert_ok!(Staking::nominate(Origin::signed(controller), vec![account_from(11)]));
		}
		mock::start_era(1);
		...
		assert_ok!(Staking::payout_nominator(Origin::signed(account_from(20_000)), 1, vec![(account_from(11), 0)]));
