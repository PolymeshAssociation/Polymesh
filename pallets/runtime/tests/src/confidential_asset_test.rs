...
fn account_create_tx() {
    ExtBuilder::default()
        .build()
        .execute_with(account_create_tx_valid_asset_id);
}

fn account_create_tx_valid_asset_id() {
    ...
    let stored_account = ConfidentialAsset::mercat_account(alice_id, wrapped_enc_asset_id.clone());

    assert_eq!(
        stored_account.clone().encrypted_asset_id,
        wrapped_enc_asset_id,
    );
    assert_eq!(
