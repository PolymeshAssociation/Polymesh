...
type BridgeTxDetail = GBridgeTxDetail<u128, BlockNumber>;

const AMOUNT: u128 = 1_000_000_000;
const AMOUNT_OVER_LIMIT: u128 = 1_000_000_000_000_000_000_000;
// const WEIGHT_EXPECTED: u64 = 500_000_210u64;
const WEIGHT_EXPECTED: u64 = 950_000_000u64;
...
fn advance_block_and_verify_alice_balance(offset: BlockNumber, expected_balance: u128) -> Weight {
    (0..=offset)
        .map(|_| {
...
fn next_unlock_block_number() -> BlockNumber {
...
fn ensure_tx_status(
    recipient: AccountId,
    nonce: u32,
    expected_status: BridgeTxStatus,
) -> BlockNumber {
...
fn do_force_mint(signers: &[AccountId]) {
    ...
    assert_eq!(alice_balance(), starting_alices_balance + AMOUNT_OVER_LIMIT);
    let execution_block = ensure_tx_status(alice.clone(), 1, BridgeTxStatus::Handled);
    assert_eq!(execution_block, unlock_block_number);
}
...
fn change_admin() {
    test_with_controller(&|signers| {
        let new_admin = signers.iter().next().unwrap().clone();
...
fn change_timelock() {
    test_with_controller(&|signers| {
        let no_admin = Origin::signed(signers.iter().next().unwrap().clone());
