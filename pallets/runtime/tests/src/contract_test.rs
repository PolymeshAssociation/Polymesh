...
/// Creates a `TestExternalities` using the given `network_fee_share` and `protocol_base_fees`.
///
...
fn check_put_code_functionality() {
    ...
    make_externalities(Perbill::from_percent(0), protocol_fee).execute_with(|| {
...
fn allow_network_share_deduction() {
    ...
    make_externalities(Perbill::from_percent(25), protocol_fee).execute_with(|| {
...
fn check_behavior_when_instantiation_fee_changes() {
    ...
    make_externalities(Perbill::from_percent(30), protocol_fee).execute_with(|| {
        let instantiation_fee = 5000;
        let fee_collector = account_from(5000);
        let alice = AccountKeyring::Alice.public();
        // Create Alice account & the identity for her.
