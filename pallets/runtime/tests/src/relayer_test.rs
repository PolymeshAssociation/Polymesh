...
fn set_call_metadata(pallet: &'static str, name: &'static str) {
    StoreCallMetadata::<TestStorage>::set_call_metadata(pallet.into(), name.into());
}

fn call_balance_transfer(val: Balance) -> <TestStorage as frame_system::Config>::Call {
    set_call_metadata("Balances", "transfer");
...
fn call_asset_register_ticker(name: &[u8]) -> <TestStorage as frame_system::Config>::Call {
    set_call_metadata("Asset", "register_ticker");
...
fn setup_subsidy(user: User, payer: User, limit: Balance) {
    ...
    exec_ok!(Relayer::set_paying_key(payer.origin(), user.acc(), limit));
    ...
    exec_ok!(Relayer::accept_paying_key(user.origin(), auth_id));
...
fn do_basic_relayer_paying_key_test() {
    ...
    exec_ok!(Relayer::set_paying_key(
        alice.origin(),
        bob.acc(),
        10 * POLY
    ));
    ...
    exec_ok!(Relayer::accept_paying_key(bob.origin(), auth_id));

    // Bob now has a subsidy of 10 POLYX.
    assert_subsidy(bob, Some((alice, 10 * POLY)));
    ...
    exec_noop!(
        Relayer::update_polyx_limit(bob.origin(), bob.acc(), 12345 * POLY),
        Error::NotPayingKey
    );

    // Alice updates the Polyx limit for Bob.  Allowed
    TestStorage::set_current_identity(&alice.did);
    exec_ok!(Relayer::update_polyx_limit(
        alice.origin(),
        bob.acc(),
        10_000 * POLY
    ));
    ...
    assert_subsidy(bob, Some((alice, 10_000 * POLY)));
    ...
    TestStorage::set_current_identity(&alice.did);
    exec_ok!(Relayer::remove_paying_key(
        alice.origin(),
        bob.acc(),
        alice.acc(),
    ));
    ...
    exec_noop!(
        Relayer::update_polyx_limit(alice.origin(), bob.acc(), 42 * POLY),
        Error::NoPayingKey
    );
...
fn do_accept_new_paying_key_test() {
    ...
    setup_subsidy(bob, alice, 10 * POLY);
    assert_usages(1, 1, 0);
    ...
    assert_subsidy(bob, Some((dave, 200 * POLY)));
...
fn do_user_remove_paying_key_test() {
    ...
    setup_subsidy(bob, alice, 200 * POLY);
...
fn do_relayer_accept_cdd_and_fees_test() {
    ...
    exec_ok!(Relayer::set_paying_key(alice.origin(), bob.acc(), 0u128));
