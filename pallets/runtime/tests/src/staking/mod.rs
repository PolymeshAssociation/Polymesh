...
use sp_runtime::{
    ...
    Perbill, Percent, Perquintill, Rounding,
};
use sp_staking::{
...
fn change_controller_works() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_add_permissioned_validator!(&stash);
...
fn nominating_and_rewards_should_work() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert_add_permissioned_validator!(&11);
            assert_ok!(Staking::validate(
                RuntimeOrigin::signed(11),
                Default::default()
            ));
            assert_add_permissioned_validator!(&31);
            assert_ok!(Staking::validate(
...
fn nominators_also_get_slashed_pro_rata() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        // assert_eq!(
        //     Staking::ledger(101).unwrap().active,
        //     nominator_stake - nominator_share
        // );
        assert_eq!(
            Staking::ledger(11).unwrap().active,
...
fn bond_with_no_staked_value() {
    ...
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            // Polymesh change
            // -----------------------------------------------------------------
            // The assertion below would fail as we have exestential deposit = 0 (Hard coded)
            // Can't bond with 1
            //assert_noop!(
            //    Staking::bond(
            //        RuntimeOrigin::signed(1),
...
fn retroactive_deferred_slashes_two_eras_before() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            on_offence_in_era(
                &[OffenceDetails {
                    offender: (11, exposure_11_at_era1),
                    reporters: vec![],
                }],
                &[Perbill::from_percent(10)],
...
fn test_max_nominator_rewarded_per_validator_and_cant_steal_someone_else_reward() {
    ExtBuilder::default().build_and_execute(|| {
        for i in 0..=<<Test as Config>::MaxNominatorRewardedPerValidator as Get<u32>>::get() as i32
        {
...
fn payout_stakers_handles_weight_refund() {
    ...
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            let call = TestCall::Staking(StakingCall::payout_stakers {
                validator_stash: 11,
                era: 1,
            });
...
fn payout_to_any_account_works() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            let _ = current_total_payout_for_duration(reward_time_per_era());
            mock::start_active_era(2);
            assert_ok!(Staking::payout_stakers(RuntimeOrigin::signed(1234), 11, 1));
...
fn cannot_rebond_to_lower_than_ed() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            // Polymesh change
            // -----------------------------------------------------------------
            assert_ok!(Staking::rebond(RuntimeOrigin::signed(21), 5),);
            // -----------------------------------------------------------------
        })
}
...
fn cannot_bond_extra_to_lower_than_ed() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            // Polymesh change
            // -----------------------------------------------------------------
            assert_ok!(Staking::bond_extra(RuntimeOrigin::signed(21), 5),);
            // -----------------------------------------------------------------
        })
}
    ...
    fn set_minimum_active_stake_is_correct() {
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                assert_ok!(<Staking as ElectionDataProvider>::electing_voters(Some(4)));
    ...
    fn set_minimum_active_stake_lower_bond_works() {
        ...
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                let voters_before =
                    <Staking as ElectionDataProvider>::electing_voters(None).unwrap();
    ...
    fn voters_include_self_vote() {
        ExtBuilder::default().nominate(false).build_and_execute(|| {
            assert!(<Validators<Test>>::iter()
                .map(|(x, _)| x)
                .all(|v| Staking::electing_voters(None)
                    .unwrap()
                    .into_iter()
                    .any(|(w, _, t)| { v == w && t[0] == w })))
        })
...
mod sorted_list_provider {
    ...
    fn re_nominate_does_not_change_counters_or_list() {
        ExtBuilder::default().nominate(true).build_and_execute(|| {
            ...
            // Polymesh change
            // -----------------------------------------------------------------
            // UseNominatorsAndValidatorsMap does not provide nominators is sorted order
            // assert_eq!(
            //    <Test as Config>::VoterList::iter().collect::<Vec<_>>(),
            //    vec![11, 21, 31, 101]
            // );
...
fn pre_bonding_era_cannot_be_claimed() {
    ...
    ExtBuilder::default().nominate(false).build_and_execute(|| {
        ...
        let claimed_rewards: BoundedVec<_, _> = (start_reward_era..=last_reward_era)
            .collect::<Vec<_>>()
            .try_into()
            .unwrap();
        assert_eq!(
...
fn add_nominator_without_cdd() {
    ExtBuilder::default().nominate(true).build_and_execute(|| {
        let alice_acc = 500;
        let alice_controller_signed = RuntimeOrigin::signed(alice_acc);
        let (_, _) =
...
fn validate_nominators_with_valid_cdd() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            let eve_acc = 700;
            let eve_controller_signed = Origin::signed(eve_acc);
            let (_, _) = make_account_with_balance(
                eve_acc,
                1_000_000,
...
fn should_initialize_stakers_and_validators() {
    // Verifies initial conditions of mock
    ExtBuilder::default().build_and_execute(|| {
        assert_eq!(Staking::bonded(&11), Some(11)); // Account 11 is stashed and locked, and account 10 is the controller
        assert_eq!(Staking::bonded(&21), Some(21)); // Account 21 is stashed and locked, and account 20 is the controller
...
fn voting_for_pip_overlays_with_staking() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        // Bond all but 10.
        assert_ok!(Staking::bond(alice_signer, 90, RewardDestination::Stash));
...
fn check_slashing_for_different_switches() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_eq!(Balances::free_balance(3), 1800);
...
fn chill_from_governance() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            bond_validator_with_intended_count(50, 500000, Some(2));
            ...
            // Add a new validator successfully.
            bond_validator_with_intended_count(70, 500000, Some(2));
...
fn test_running_count() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert_ok!(Staking::chill(Origin::signed(60)));
...
fn validator_unbonding() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert_noop!(
                Staking::unbond(Origin::signed(50), 180_000),
                Error::<Test>::InsufficientBond
            );
            // Chill validator
            assert_ok!(Staking::chill(Origin::signed(50)));
            // After chilling validator checks that entity can unbond successfully
            assert_ok!(Staking::unbond(Origin::signed(50), 180_000));
        });
}
