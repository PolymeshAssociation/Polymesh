...
fn force_unstake_works() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_noop!(
            Staking::force_unstake(RuntimeOrigin::signed(11), 11, 2),
            BadOrigin
        );
...
fn rewards_should_work() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert_eq!(
                asset::total_issuance::<Test>(),
                total_issuance_1 + total_payout_1
            );
            assert_eq_error_rate!(
                asset::total_balance::<Test>(&11),
...
fn less_than_needed_candidates_works() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert!(
                ErasStakersPaged::<Test>::iter_prefix_values((active_era(),))
...
fn nominating_and_rewards_should_work() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert_eq!(
                ErasStakersPaged::<Test>::iter_prefix_values((active_era(),)).count(),
                2
            );
            assert_eq!(
                Staking::eras_stakers(active_era(), &11),
...
fn reward_destination_works() {
    ...
    ExtBuilder::default().nominate(false).build_and_execute(|| {
        ...
        assert_eq!(
            Staking::payee(11.into()),
            Some(RewardDestination::Account(11))
        );
        // Check that reward went to the controller account
        assert_eq!(
            asset::stakeable_balance::<Test>(&11),
            recorded_stash_balance + total_payout_2
        );
        // Check that amount at stake is NOT increased
        assert_eq!(
            Staking::ledger(11.into()).unwrap(),
...
fn bond_extra_controller_bad_state_works() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            assert_eq!(
                StakingLedger::<Test>::get(StakingAccount::Stash(31))
                    .unwrap()
                    .stash,
                31
            );
...
fn auto_withdraw_may_not_unlock_all_chunks() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        // `BondingDuration`
        assert_noop!(
            Staking::unbond(RuntimeOrigin::signed(11), 1),
...
fn max_staked_rewards_default_works() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_eq!(
            ErasValidatorReward::<Test>::get(0).unwrap(),
            default_stakers_payout
        );
...
fn max_staked_rewards_works() {
    ExtBuilder::default().nominate(true).build_and_execute(|| {
        ...
        assert_eq!(
            validators_payout,
            Percent::from_percent(max_staked_rewards) * total_payout
        );
...
fn test_multi_page_payout_stakers_by_page() {
    ...
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert!(
                pallet_balances::TotalIssuance::<Test>::get() < pre_payout_total_issuance + payout
            );
            ...
            for era in 0..15 {
                assert_eq!(
                    ClaimedRewards::<Test>::get(era, &11),
                    Vec::<sp_staking::Page>::new()
                );
            }
            ...
            assert_eq!(
                ClaimedRewards::<Test>::get(first_claimable_reward_era, &11),
                vec![0]
            );
            ...
            assert_eq!(
                ClaimedRewards::<Test>::get(last_reward_era, &11),
                vec![0, 1]
            );
...
fn test_page_count_and_size() {
    ...
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert_eq!(
                EraInfo::<Test>::get_paged_exposure(1, &11, 0)
                    .unwrap()
                    .others()
                    .len(),
                64
            ...
            assert_eq!(
                EraInfo::<Test>::get_paged_exposure(1, &11, 1)
                    .unwrap()
                    .others()
                    .len(),
                36
            ...
            assert_eq!(
                EraInfo::<Test>::get_paged_exposure(2, &11, 0)
                    .unwrap()
                    .others()
                    .len(),
                32
            ...
            assert_eq!(
                EraInfo::<Test>::get_paged_exposure(2, &11, 1)
                    .unwrap()
                    .others()
                    .len(),
                32
            ...
            assert_eq!(
                EraInfo::<Test>::get_paged_exposure(2, &11, 2)
                    .unwrap()
                    .others()
                    .len(),
                32
            ...
            assert_eq!(
                EraInfo::<Test>::get_paged_exposure(2, &11, 3)
                    .unwrap()
                    .others()
                    .len(),
                4
...
fn restricted_accounts_can_only_withdraw() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        start_active_era(2);
        assert_noop!(
            Staking::rebond(RuntimeOrigin::signed(alice), 50),
            Error::<Test>::Restricted
        );
    ...
    fn voters_include_self_vote() {
        ExtBuilder::default().nominate(false).build_and_execute(|| {
            ...
            assert!(<Validators<Test>>::iter()
                .map(|(x, _)| x)
                .all(
                    |v| Staking::electing_voters(DataProviderBounds::default(), SINGLE_PAGE)
                        .unwrap()
                        .into_iter()
                        .any(|(w, _, t)| { v == w && t[0] == w })
                ))
    ...
    fn nominations_quota_limits_size_work() {
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                // -----------------------------------------------------------------
                // We don't care about the order
                let elected_voters = Staking::electing_voters(bounds.voters, SINGLE_PAGE)
                    .unwrap()
                    .iter()
                    .map(|(stash, _, targets)| (*stash, targets.len()))
                    .collect::<Vec<_>>();
...
fn change_of_absolute_max_nominations() {
    ...
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert_eq!(
                Staking::electing_voters(bounds, SINGLE_PAGE).unwrap().len(),
                3 + 3
            );
...
fn nomination_quota_max_changes_decoding() {
    ...
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert_eq!(
                Staking::electing_voters(unbonded_election, SINGLE_PAGE)
                    .unwrap()
                    .len(),
                4 + 4
            );
...
fn api_nominations_quota_works() {
    ExtBuilder::default().build_and_execute(|| {
        assert_eq!(
            Staking::api_nominations_quota(10),
            MaxNominationsOf::<Test>::get()
        );
        assert_eq!(
            Staking::api_nominations_quota(333),
            MaxNominationsOf::<Test>::get()
        );
        assert_eq!(Staking::api_nominations_quota(222), 2);
...
fn force_apply_min_commission_works() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_eq!(
            validators(),
            vec![(31, prefs(10)), (21, prefs(5)), (11, prefs(5))]
        );
...
fn proportional_ledger_slash_works() {
    ExtBuilder::default().nominate(true).build_and_execute(|| {
        ...
        assert_eq!(
            ledger.unlocking,
            vec![c(4, 100), c(5, 100), c(6, 30), c(7, 30)]
        );
        assert_eq!(ledger.total, 4 * 100 - 140);
        assert_eq!(LedgerSlashPerEra::get().0, 0);
        assert_eq!(
            LedgerSlashPerEra::get().1,
            BTreeMap::from([(6, 30), (7, 30)])
        );
        ...
        assert_eq!(
            ledger.unlocking,
            vec![c(4, 100), c(5, 100), c(6, 100 - 8), c(7, 100 - 7)]
        );
        assert_eq!(ledger.total, 4 * 100 - 15);
        assert_eq!(LedgerSlashPerEra::get().0, 0);
        assert_eq!(
            LedgerSlashPerEra::get().1,
            BTreeMap::from([(6, 92), (7, 93)])
        );
...
fn reducing_max_unlocking_chunks_abrupt() {
    ...
    ExtBuilder::default().build_and_execute(|| {
        ...
        assert_noop!(
            Staking::unbond(RuntimeOrigin::signed(3), 20),
            Error::<Test>::NoMoreChunks
        );
        ...
        assert_noop!(
            Staking::rebond(RuntimeOrigin::signed(3), 100),
            Error::<Test>::NotController
        );
...
fn can_page_exposure() {
    ...
    let exposure: Exposure<AccountId, Balance> = Exposure {
        total: total_stake,
        own: own_stake,
        others,
    };
...
fn should_retain_era_info_only_upto_history_depth() {
    ExtBuilder::default().build_and_execute(|| {
        ...
        for i in 0..4 {
            ...
            assert_eq!(
                ClaimedRewards::<Test>::iter_prefix(i as EraIndex).count(),
                1
            );
            // 3 entries (pages) for each era
            assert_eq!(
                ErasStakersPaged::<Test>::iter_prefix((i as EraIndex,)).count(),
                3
            );
...
fn test_validator_exposure_is_backward_compatible_with_non_paged_rewards_payout() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            bond_validator(11, 1000);
            let mut expected_individual_exposures: Vec<IndividualExposure<AccountId, Balance>> =
                vec![];
...
mod staking_unchecked {
    ...
    fn migrate_virtual_staker() {
        ExtBuilder::default().build_and_execute(|| {
            ...
            assert_ok!(<Staking as StakingUnchecked>::migrate_to_virtual_staker(
                &200
            ));
    ...
    fn restore_ledger_not_allowed_for_virtual_stakers() {
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                assert_eq!(
                    Staking::inspect_bond_state(&333).unwrap(),
                    LedgerIntegrityState::Corrupted
                );
...
mod ledger {
    ...
    fn paired_account_works() {
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                assert_eq!(
                    StakingLedger::<Test>::paired_account(StakingAccount::Stash(10)),
                    Some(10)
                );

                assert_eq!(<Bonded<Test>>::get(&42), None);
                assert_eq!(
                    StakingLedger::<Test>::paired_account(StakingAccount::Controller(42)),
                    None
                );
                assert_eq!(
                    StakingLedger::<Test>::paired_account(StakingAccount::Stash(42)),
                    None
                );
    ...
    fn bond_controller_cannot_be_stash_works() {
        ExtBuilder::default().build_and_execute(|| {
            ...
            assert_eq!(
                Ledger::<Test>::get(controller).map(|l| l.stash),
                Some(stash)
            );
    ...
    fn is_bonded_works() {
        ExtBuilder::default().build_and_execute(|| {
            assert!(!StakingLedger::<Test>::is_bonded(StakingAccount::Stash(42)));
            assert!(!StakingLedger::<Test>::is_bonded(
                StakingAccount::Controller(42)
            ));
    ...
    fn inspect_recovery_ledger_corupted_killed_works() {
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                assert_eq!(
                    Staking::inspect_bond_state(&333),
    ...
    fn restore_ledger_corrupted_bond_extra_works() {
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                let ledger_333 = Bonded::<Test>::get(&333)
                    .and_then(Ledger::<Test>::get)
                    .unwrap();
                let ledger_444 = Bonded::<Test>::get(&444)
                    .and_then(Ledger::<Test>::get)
                    .unwrap();
        ...
        ExtBuilder::default()
            ...
            .build_and_execute(|| {
                ...
                let pre_upgrade_result =
                    v16::VersionUncheckedMigrateV15ToV16::<Test>::pre_upgrade();
...
fn manual_slashing_works() {
    ExtBuilder::default()
        ...
        .build_and_execute(|| {
            ...
            assert!(
                initial_balance > 0,
                "Validator must have stake to be slashed"
            );
            ...
            let no_slashed_events = !System::events().iter().any(|record| {
                matches!(
                    record.event,
                    RuntimeEvent::Staking(Event::<Test>::Slashed { .. })
                )
            });
            assert!(
                no_slashed_events,
                "A Slashed event was incorrectly emitted immediately"
            );
