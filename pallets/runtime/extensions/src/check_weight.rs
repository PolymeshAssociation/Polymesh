...
use frame_system::{limits::BlockWeights, Config, ConsumedWeight, Module};
use sp_runtime::{
    traits::{DispatchInfoOf, Dispatchable, PostDispatchInfoOf, Printable, SignedExtension},
    ...
    T::Call: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
{
    ...
    fn check_block_weight(
        info: &DispatchInfoOf<T::Call>,
    ) -> Result<ConsumedWeight, TransactionValidityError> {
    ...
    pub fn do_pre_dispatch(
        ...
        Module::<T>::set_all_extrinsics_len(next_len);
        Module::<T>::set_block_weight(next_weight);
        Ok(())
    }
    ...
    T::Call: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
{
    ...
    fn post_dispatch(
        ...
        if unspent > 0 {
            mutate_block_weight::<T>(unspent, info.class);
        }
...
pub(crate) fn mutate_block_weight<T: Config>(weight: Weight, class: DispatchClass) {
    let mut current_weight = System::<T>::block_weight();
    current_weight.set(weight, class);
    System::<T>::set_block_weight(current_weight);
}
...
mod tests {
    ...
    fn full_block_with_normal_and_operational() {
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(
                CheckWeight::<Test>::check_extrinsic_weight(&rest_operational),
                Ok(())
            );
        });
    }
    ...
    fn signed_ext_check_weight_works_operational_tx() {
        new_test_ext().execute_with(|| {
            ...
            mutate_block_weight(normal_limit, DispatchClass::Normal);

            // will not fit.
            assert!(CheckWeight::<Test>(PhantomData)
                .pre_dispatch(&1, CALL, &normal, len)
                .is_err());
            // will fit.
            assert!(CheckWeight::<Test>(PhantomData)
                .pre_dispatch(&1, CALL, &op, len)
                .is_ok());

            // likewise for length limit.
            let len = 100_usize;
            Module::<Test>::set_all_extrinsics_len(normal_length_limit());
            assert!(CheckWeight::<Test>(PhantomData)
                .pre_dispatch(&1, CALL, &normal, len)
                .is_err());
            assert!(CheckWeight::<Test>(PhantomData)
    ...
    fn signed_ext_check_weight_block_size_works() {
        new_test_ext().execute_with(|| {
            ...
            let reset_check_weight = |tx, s, f| {
                Module::<Test>::set_all_extrinsics_len(0);
    ...
    fn signed_ext_check_weight_works_normal_tx() {
        new_test_ext().execute_with(|| {
            ...
            let reset_check_weight = |i, f, s| {
                mutate_block_weight(s, DispatchClass::Normal);
    ...
    fn signed_ext_check_weight_refund_works() {
        new_test_ext().execute_with(|| {
            ...
            mutate_block_weight(0, DispatchClass::Mandatory);
            mutate_block_weight(256 - base_extrinsic, DispatchClass::Normal);
            ...
            assert!(
                CheckWeight::<Test>::post_dispatch(pre, &info, &post_info, len, &Ok(())).is_ok()
            );
            assert_eq!(
    ...
    fn signed_ext_check_weight_actual_weight_higher_than_max_is_capped() {
        new_test_ext().execute_with(|| {
            ...
            mutate_block_weight(0, DispatchClass::Mandatory);
            mutate_block_weight(128, DispatchClass::Normal);
    ...
    fn normal_and_mandatory_tracked_separately() {
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(
                CheckWeight::<Test>::check_extrinsic_weight(&mandatory),
                Ok(())
            );
        });
    }
