...
use polymesh_primitives::{
    ...
    Context, IdentityId, Proposition, Ticker, TrustedFor, TrustedIssuer,
};
use sp_std::{
...
type ExternalAgent<T> = pallet_external_agents::Module<T>;
...
pub struct AnyEAProp<T>(Ticker, PhantomData<T>);

impl<C, T: EATrait> Proposition<C> for AnyEAProp<T> {
    fn evaluate(&self, context: Context<C>) -> bool {
        ExternalAgent::<T>::agents(self.0, context.id).is_some()
    }
}
...
impl<T: Trait> Module<T> {
    ...
    fn is_condition_satisfied(
        ...
        proposition::run(&condition, context, AnyEAProp::<T>(*ticker, PhantomData))
    }
    ...
    fn evaluate_conditions(
        ...
        conditions.iter_mut().fold(true, |result, condition| {
            ...
            let context = Self::fetch_context(did, ticker, issuers, cond);
            condition.result =
                proposition::run(cond, context, AnyEAProp::<T>(*ticker, PhantomData));
