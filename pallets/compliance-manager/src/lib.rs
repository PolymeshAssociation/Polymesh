...
use polymesh_primitives::{
    ...
    proposition::{self, IdentityClaims},
    storage_migration_ver, Claim, Condition, ConditionType, IdentityId, Ticker, TrustedFor,
    TrustedIssuer,
};
...
impl<T: Config> Module<T> {
    /// Returns all non expired claims of type `ClaimType` and scope of `Scope` that were issued by
    /// a trusted issuer for the given `id`.
    fn fetch_claims(
        id: IdentityId,
        claim: &Claim,
        ...
        for issuer in trusted_issuers {
            // Ignores issuers not trusted for this claim
            if !issuer.is_trusted_for(claim_type) {
                continue;
            }
            // Gets non expired claims issued by the issuer
            if let Some(identity_claim) =
                Identity::<T>::fetch_claim(id, claim_type, issuer.issuer, scope.cloned())
            {
                claims.push(identity_claim.claim)
            }
        }
        claims
    }

    /// Returns the trusted issuers for the given `condition`.
    /// If it doesn't have any, the asset's trusted issuers is returned.
    fn get_trusted_issuers(condition: &Condition, ticker: &Ticker) -> Vec<TrustedIssuer> {
    ...
    /// Returns all claims from the trusted issuers for the given identity.
    fn fetch_identity_claims(
        id: IdentityId,
        ticker: &Ticker,
        condition: &Condition,
    ) -> IdentityClaims {
        let claims = {
            match &condition.condition_type {
                ConditionType::IsPresent(claim) | ConditionType::IsAbsent(claim) => {
                    let trusted_issuers = Self::get_trusted_issuers(condition, ticker);
                ...
                ConditionType::IsAnyOf(claims) | ConditionType::IsNoneOf(claims) => {
                    ...
                    claims
                        .iter()
                        .flat_map(|claim| Self::fetch_claims(id, claim, &trusted_issuers))
                        .collect()
                }
                ConditionType::IsIdentity(_) => Vec::new(),
            }
        };
    ...
    fn are_all_conditions_satisfied(
        ...
        conditions
            .iter()
            .all(|condition| Self::is_condition_satisfied(did, ticker, condition))
    }

    /// Returns `true` if the condition is satisfied.
    fn is_condition_satisfied(id: IdentityId, ticker: &Ticker, condition: &Condition) -> bool {
        let identity_claims = Self::fetch_identity_claims(id, ticker, condition);
        let has_external_agents = |id_claims: &IdentityClaims| {
            ExternalAgents::<T>::agents(ticker, id_claims.id).is_some()
        };
        proposition::run(&condition, &identity_claims, has_external_agents)
    }
    ...
    fn evaluate_conditions(
        ...
        conditions.iter_mut().fold(true, |overall, res| {
            res.result = Self::is_condition_satisfied(did, ticker, &res.condition);
            overall & res.result
        })
    }
