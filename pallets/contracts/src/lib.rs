...
impl<T: Trait> Module<T> {
    ...
    fn restore_to(
        ...
        let key_values_taken = delta
            .iter()
            .filter_map(|key| {
                child::get_raw(&origin_contract.child_trie_info(), &blake2_256(key)).map(|value| {
                    child::kill(&origin_contract.child_trie_info(), &blake2_256(key));
        ...
        let tombstone = <TombstoneContractInfo<T>>::new(
            ...
            &child::root(&origin_contract.child_trie_info())[..],
            code_hash,
        );

        if tombstone != dest_tombstone {
            for (key, value) in key_values_taken {
                child::put_raw(&origin_contract.child_trie_info(), &blake2_256(key), &value);
            }
