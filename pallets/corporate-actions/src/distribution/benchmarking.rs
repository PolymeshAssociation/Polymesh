...
fn dist<T: Trait + TestnetFn<AccountIdOf<T>>>(target_ids: u32) -> (User<T>, CAId, Ticker) {
...
fn prepare_transfer<T: Trait + pallet_compliance_manager::Trait + TestnetFn<AccountIdOf<T>>>(
    target_ids: u32,
    did_whts_num: u32,
) -> (User<T>, User<T>, CAId) {
