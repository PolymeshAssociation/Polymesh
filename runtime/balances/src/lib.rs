...
use polymesh_primitives::{IdentityId, Key, Permission, Signer, TransactionError};
use polymesh_runtime_common::traits::identity::IdentityTrait;
...
use runtime_primitives::{
    ...
    traits::{
        ...
        Saturating, SignedExtension, SimpleArithmetic, StaticLookup, Zero, One},
...
impl<T: Trait<I>, I: Instance + Clone + Eq> SignedExtension for TakeFees<T, I> {
    ...
    fn validate(
        ...
        if let Some(did) = <Module<T, I>>::charge_fee_to_identity(&encoded_transactor) {
            print("Charging fee to identity");
