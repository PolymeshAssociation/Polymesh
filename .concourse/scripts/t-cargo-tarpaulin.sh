...
cargo tarpaulin \
  --packages polymesh-runtime-common \
  --packages pallet-staking \
  --packages pallet-group \
  --packages polymesh-primitives \
  --packages node-rpc-runtime-api \
  --packages pallet-transaction-payment \
  --packages pallet-cdd-offchain-worker \
